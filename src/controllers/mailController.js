...
import { sendReferralVisitNotification, sendReportPurchasedNotification } from '../services/emailService.js';
...
export async function getRecipientByCode(req, res) {
  try {
    ...
    const updateResult = await pool.query(
      `UPDATE mail_recipients SET page_views = page_views + 1, last_viewed_at = NOW() WHERE short_code = $1 RETURNING page_views`,
      [code]
    );

    const pageViews = updateResult.rows[0]?.page_views || 1;

    // Send visit email notification (fire and forget)
    const ipAddress = req.headers['x-forwarded-for'] || req.socket?.remoteAddress || '';
    const userAgent = req.headers['user-agent'] || '';
    sendReferralVisitNotification(
      { address: recipient.address, short_code: recipient.shortCode, recipient_name: recipient.recipientName },
      { ip_address: ipAddress, user_agent: userAgent, visited_at: new Date().toISOString(), visit_count: pageViews }
    ).catch(() => {});

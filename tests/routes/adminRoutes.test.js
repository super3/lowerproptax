...
describe('Admin Routes', () => {
  ...
  describe('PUT /api/admin/properties/:id/mark-ready', () => {
    it('should return 401 without authentication', async () => {
      const response = await request(app).put('/api/admin/properties/prop1/mark-ready');

      expect(response.status).toBe(401);
    });

    it('should mark property as ready if user is admin', async () => {
      const mockProperty = {
        id: 'prop1',
        status: 'ready',
        updated_at: new Date()
      ...
      const response = await request(app)
        .put('/api/admin/properties/prop1/mark-ready')
        .set('Authorization', `Bearer ${validToken}`);

      expect(response.status).toBe(200);
      expect(response.body).toEqual(expect.objectContaining({ id: 'prop1', status: 'ready' }));
    });

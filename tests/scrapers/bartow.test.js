...
describe('Bartow County Scraper', () => {
  const TEST_ADDRESS = '20 Allweather St';
  const COUNTY = 'bartow';
  ...
  test('should return correct county', () => {
    if (result === null) return;
  ...
  test('should return homestead exemption status (or null if not listed)', () => {
    if (result === null) return;
  ...
  test('should return tax record URL (or null if not available)', () => {
    if (result === null) return;
    if (result.taxRecordUrl !== null) {
      expect(result.taxRecordUrl).toBeTruthy();
    } else {
      expect(result.taxRecordUrl).toBeNull();
    }
  });
});
